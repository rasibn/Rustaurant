...
pub fn search_input(props: &Props) -> Html {
    ...
    let onsubmit = Callback::from(move |e: MouseEvent| {
        e.prevent_default();
        web_sys::console::log_1(&format!("Hi from Search {:?}", search_value).into());
        // props.onsubmit.emit(return_value.clone());
    });
    ...
    <button type="submit" onclick={onsubmit} class="p-2.5 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
